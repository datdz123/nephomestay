 ...
 */
//Css Admin
if ( ! function_exists( 'qd_css_admin' ) ) :
	add_action( 'admin_head', 'qd_css_admin' );
	add_action( 'admin_enqueue_scripts', 'qd_css_admin' );
	function qd_css_admin() {
		wp_enqueue_style( 'admin_css', get_template_directory_uri() . '/admin/admin.css' );
	}
endif;
//CSS Login
if ( ! function_exists( 'qd_css_admin_login' ) ) :
	add_action( 'login_enqueue_scripts', 'qd_css_admin_login' );
	function qd_css_admin_login() {
 ...
 */
add_action( 'add_attachment', 'qd_set_image_meta_image_upload' );
function qd_set_image_meta_image_upload( $post_ID ) {
	if ( wp_attachment_is_image( $post_ID ) ) {
		$qd_image_title = get_post( $post_ID )->post_title;
		$qd_image_title = preg_replace(
			'%\s*[-_\s]+\s*%',
			' ',
			$qd_image_title
		);
		$qd_image_title = ucwords( strtolower( $qd_image_title ) );
		$qd_my_image_meta = array(
			'ID' => $post_ID,
			'post_title' => $qd_image_title,
			'post_excerpt' => '',
			'post_content' => '',
		);
		update_post_meta( $post_ID, '_wp_attachment_image_alt', $qd_image_title );
		wp_update_post( $qd_my_image_meta );
	}
}
 ...
 */
function qd_remove_version() {
	return '';
}
add_filter( 'the_generator', 'qd_remove_version' );
...
add_action( 'admin_head', 'qd_remove_help_tabs' );

function qd_remove_help_tabs() {
